...
alias rg='rg --no-heading --hidden --glob "!.git" --smart-case --colors "path:fg:blue" --colors "match:fg:green" --colors "line:fg:yellow" --sort path'
alias gti='git'
alias g='git'
alias maek='make'
...
gct() {
  ...
  elif ! git remote get-url "$REMOTE" &>/dev/null; then
    local ORIGIN_URL="$(git remote get-url origin)" REMOTE_URL
    if [[ "$ORIGIN_URL" == *://* ]]; then
